...
function GetSubMenu ($id, $table, $getCol, $tid, $parent) {
	$sql = "SELECT $tid, $getCol FROM $table WHERE `parent`=$parent ORDER BY `sort`";
	$res = safe_query($sql);

	$pd = '<option value="0">bitte wählen</option>';

	while ($row = mysqli_fetch_object($res)) {
		if ($row->$tid == $id) $sel = "selected";
		else $sel = "";

		$nm = $row->$getCol;
		$pd .= "<option value=\"" .$row->$tid ."\" $sel>$nm</option>\n";
	}
	return $pd;
}
...
#				elseif (preg_match("/menu/", $b_chk)) 									$insert_menu = 1;
				elseif (preg_match("/material/", $b_chk)) 								$insert_material = 1;
				elseif (preg_match("/farbpalette/", $b_chk)) 							$insert_farbpalette = 1;
				elseif (preg_match("/oberfl/", $b_chk)) 								$insert_oberflaeche = 1;
				...
				elseif (preg_match("/masterslider/", $b_chk)) 							$insert_masterslider = 1;
...
//				elseif (preg_match("/termin/", $b_chk)) 								$insert_termin = 1;
				...
				elseif (preg_match("/html/", $b_chk) || preg_match("/termine tab/", $b_chk) || preg_match("/themenliste/", $b_chk)) 								$row = 10;
...
# # ________________________________________________________________________________________________________________________________
				...
				if ($image) {
					...
					if($w < $height) 	$blob 	 = "<img src=\"".$img_pfad.urlencode($imgname).'"'.($isSVG ? ' style="height:'.$height.'; width:'.$height.'px; margin: 0 30px;"' : '').'>';
					else 			{ $height=60; $blob = "<img src=\"../mthumb.php?src=images/userfiles/image/".urlencode($imgname)."&h=$height\" class=\"img-responsive\" border=1>";  }
					...
					<table id="t'.$counter.'" class="item" style="min-height:'.$height.'px;">'."
						<tr>
...
<p style=\"margin: 10px 0px 0px 0px;\"><i class=\"fa fa-trash-o del\" ref=\"t".$counter."\"></i></p></td>
				...
				elseif ($insert_form || $insert_templ || $insert_termin || $insert_news || $insert_loesung || $galerie || $insert_branche || $insert_pdf || $insert_shop || $insert_anwendung || $insert_mindflash || $insert_kunde || $insert_mitarbeiter || $insert_objekt || $insert_presse || $insert_video || $insert_menu || $insert_vorlage || $insert_warengruppe || $insert_land || $insert_farbe || $insert_icon || $insert_colour || $insert_klasse || $insert_pdf_group || $insert_buchung || $insert_referenz || $insert_masterslider || $insert_material || $insert_oberflaeche || $insert_farbpalette) {
					...
					elseif ($insert_material) {
						unset($insert_form);
						$pdf_pd = GetSubMenu ($txt, "morp_cms_nav", "name", "navid", 48);
					}
					elseif ($insert_oberflaeche) {
						unset($insert_form);
						$pdf_pd = GetSubMenu ($txt, "morp_cms_nav", "name", "navid", 49);
					}
					elseif ($insert_farbpalette) {
						unset($insert_farbpalette);
						$pdf_pd = pulldownContent ($txt, "morp_cms_img_group", "name", "gid");
					}
					elseif ($insert_form) {
						unset($insert_form);
					...
					elseif ($insert_masterslider) {
						unset($insert_masterslider);
						$pdf_pd = pulldownContent ($txt, "morp_masterslider", "msName", "msID");
					}
					...
					elseif ($insert_video) {
						unset($insert_video);
						$pdf_pd = pulldownContent ($txt, "morp_cms_pdf", "pname", "pid", "2");
					}
...
# content_edit.php?db=$db&edit=$edit&stelle=$counter&split=$counter&save=1
					...
					$textbox .= "
							<td>
								<textarea id=\"s".$counter."\" name=\"brick#" .$brick[0] ."#" .$counter ."\" style=\"height:" .($row*30) ."px;\" class=\"form-control ta gridcheck getVal \" onchange=\"setchange('1');\">".stripslashes($txt)."</textarea>
							</td>
						</tr>
					</table>
					";
...
<div class="panel panel-default fixTop panel-edit">
  <div class="panel-heading">
	<ul class="navTopTop">
		...
		'.($muuri ? '
		<li class="muuri"><a href="?edit='.$edit.'&navid='.$navid.'&muuri=off" title="Drag and Drop off"><img src="images/drag-drop-off.png" class="icon"></a></li>' : '
		<li class="muuri"><a href="?edit='.$edit.'&navid='.$navid.'&muuri=on" title="Drag and Drop on"><img src="images/drag-drop-div.png" class="icon"></a></li>').'
	</ul>
  </div>
...
echo '
	<ul class="pdfmap">
		<li class="closeHeaderPDF"><i class="fa fa-close"></i></li>
';

// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + 
// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + 
// INSERT LIST OF ALL PDF
$sql_pdf = "SELECT pid, pname FROM morp_cms_pdf WHERE 1";
$res_pdf = safe_query($sql_pdf);
while ($row_pdf = mysqli_fetch_object($res_pdf)) {
	echo '<li><a data-clipboard-text="<a:pdf#'.$row_pdf->pid.':>Link_Name</a>" class="intLink">'.$row_pdf->pname.'</a></li>';
...
echo setKeywords($navid, 'morp_cms_nav', 'content', 'navid');
...
.pdfmap { padding: 10px 20px 20px 40px; }
.sitemap a, .sitemap li, .pdfmap a, .pdfmap li {
	color: #fff;
}
.closeHeader, .closeExtLinkHeader, .closemailadrHeader, .closeHeaderPDF { background: #222; list-style: none; height: 30px; text-align: right; border: solid 1px #1997c6; width: 50px; padding: 6px 10px 5px; display: block; float: right;
}
.closeExtLinkHeader, .closemailadrHeader { background: transparent; margin-top: -50px; position: absolute; right: 5%; top: 11%; }
.closeHeader .fa, .closeHeaderPDF .fa {
	color: #1997c6;
}
.copied {
