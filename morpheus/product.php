...
if ($save) {
    $query = "update morp_cms_img_group set product = 0 where product = ".$edit."";
    safe_query($query);

    $query = "update morp_cms_img_group set product= ".$edit." where gid = ".$imageHeader."";
    safe_query($query);
...
if ($edit && !$save) {
    ...
    while ($row = mysqli_fetch_object($result)) {
        $select = ($edit == $row->product) ? "selected" : "";
...
while ($row = mysqli_fetch_object($result)) {
	...
	if ($nm) {
		...
		echo "<tr>
			...
			<td valign=\"top\">";
        echo "<a href='product.php?edit=$id'>";

		echo "$nm</a>\n";

		if ($admin && !$back && !$nid && !$kurs && $db != "template") echo "&nbsp;
			<td width=50 align=center>$lnk".'<a href="product.php?edit='.$id.'"><i class="fa fa-pencil-square-o"></i></a>'."
