...
*/
	...
	$warnung = '
			<form method=post enctype="multipart/form-data">
				...
				<input type="hidden" name="name" value="'.$row->gnname.'"></p>
	...
	<div class="conatiner-full">
		...
		<div class="row">
			...
			<div class="col-md-1 form-group">
				...
				<input type="text" name="sort" value="'.$row->sort.'"  class="form-control"></p>
			</div>

			<div class="col-md-2 form-group">
				<p>Titel Galerie<br>
				<input type="text" name="gntextde" value="'.$row->gntextde.'" class="form-control"></p>
			</div>

<!--
			<div class="col-md-3 form-group">
				<p>HTML English<br>
				<textarea cols="130" rows="16" name="texten" class="form-control">'.$row->texten.'</textarea></p>
			</div>
-->

			<div class="col-md-4 form-group">
				<p>Kurz-Beschreibung Galerie<br>
				<textarea cols="130" rows="6" name="textde" class="form-control">'.$row->textde.'</textarea></p>
			</div>

<!--
			<div class="col-md-3 form-group">
				<p style="margin: 10px 0px 10px 0px;">text English<br>
				<textarea cols="130" rows="6" name="texten" class="form-control">'.$row->texten.'</textarea></p>
			</div>
-->
			<div class="col-md-2 form-group">
				<p>'.$MORPTEXT["GLOB-DATUM"].'<br>
				<input type="text" name="date" value="'.$datum.'" class="form-control"></p>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-md-6 form-group">
				<p><img src="../mthumb.php?w=400&amp;src=Galerie/'.$row->img.'"></p>
			</div>
...
elseif ($gal == "name") {
	...
	echo '
		<p>
			...
			<a href="bild_galerie.php?reorg='.$ggid.'" class="button"><i class="fa fa-sort-alpha-asc"></i> '.$MORPTEXT["GLOB-SORT-AKT"].'</a>
		</p>
...
';
	...
	while ($row = mysqli_fetch_object($result)) {
		...
		$lnk 	= "<a href=\"bild_galerie.php?gnid=".$row->gnid."&ggid=".$ggid."&db=morp_cms_galerie\" title=\"oeffne ".$row->gnname."\"  class=\"btn btn-info bt4\">";
		$sid    = $row->sort;
		$si 	= $row->sichtbar;
		$x++;
		...
		echo '
		<div class="zeile item row col1" id="'.$row->gnid.'">
			<div class="col-md-1 col-xs-1">'.$sid.'</div>
			<div class="col-md-4 col-xs-5">'. ($vorh ? '' : '<strong style="color:red;">Ordner fehlt</strong>&nbsp;&nbsp;') .$lnk.$row->gnname." | ".$row->gntextde.'</a></div>
			<div class="col-md-1 col-xs-1"><a href="bild_galerie.php?gnid='.$row->gnid.'&ggid='.$ggid.'&db='.$db.'&dbnm='.$dbnm.'&dbid='.$dbid.'&olddir='.$row->gnname.'&ednm='.$row->gnname.'"> <i class="fa fa-cogs"></i></a></div>
			<div class="col-md-1 col-xs-1">'.$lnk.$ed.'</a> </div>
		...
		echo '</div>
			<div class="col-md-1 col-xs-1"><a name="'.deldir.'" href="bild_galerie.php?deldir='.$row->gnid.'&ggid='.$ggid.'&db=morp_cms_galerie" title="deldir" class="btn btn-danger">'.$delete.'</a></div>
...
<?php if($muuri) { ?>
  ...
  $( function() {
	...
	grid.on('dragEnd', function (item) {
	    ...
	    request = $.ajax({
	        url: "UpdatePosGalerie.php",
	        type: "post",
	        data: "data="+order+"&table=morp_cms_galerie_name&tid=gnid",
