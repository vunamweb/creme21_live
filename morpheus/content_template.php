...
if ($save) {
	$arr = array("tlink", "tbackground", "tid", "timage", "theadl", "theight", "twidth", "tcolor", "tref", "tende", "tabstand", "tschmal", "tabstand_bottom", "klasse");
	foreach ($arr as $val) {
		$set[] = $val."='".$_POST[$val]."'";
	}
	$set = implode(", ", $set);
	$sql = "UPDATE `morp_cms_content` SET $set , edit=1 WHERE cid=$edit";
	$res = safe_query($sql);
...
elseif ($ilink) {
	$sql  = "UPDATE `morp_cms_content` SET tlink='$ilink' , edit=1 WHERE cid=$edit";
	$res = safe_query($sql);
}
elseif ($imgsav) {
	$sql  = "UPDATE `morp_cms_content` SET timage='$imgsav' , edit=1 WHERE cid=$edit";
	$res = safe_query($sql);
}
elseif ($delimg) {
	$sql  = "UPDATE `morp_cms_content` SET timage='', edit=1 WHERE cid=$edit";
	$res = safe_query($sql);
}
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
...
if($arr_sp) {
echo '<select name="tref" class="form-control mb1 mt1">';
...
'.(in_array($tid, $morpheus["template_abstand"]) ? '<p>Padding am Anfang Section: &nbsp; <input type="checkbox" name="tabstand" id="tabstand" value="1"'.($tabstand ? ' checked' : '').'>' : '').'
'.(in_array($tid, $morpheus["template_abstand"]) ? '<p>Padding am Ende Section: &nbsp; <input type="checkbox" name="tabstand_bottom" id="tabstand_bottom" value="1"'.($tabstand_bottom ? ' checked' : '').'></p>' : '').'
'.(in_array($tid, $morpheus["template_schmal"]) ? '<p>Eingerückt / 1 Leerspalte: &nbsp; <input type="checkbox" name="tschmal" id="tschmal" value="1"'.($tschmal ? ' checked' : '').'></p>' : '').'
...
echo '
				<p>&nbsp;</p>
				<p class="mb1 mt1"><input type="submit" name="speichern" value="speichern"></p>
				<p class="mb1 mt1"><input type="submit" name="speichern" value="speichern und zurueck"></p>
