...
function repairNav($navid) {
	...
	foreach ($arr as $val) {
		$xx++;
		$sql  = "UPDATE `morp_cms_content` SET tpos=$xx , edit=1 WHERE cid=$val";
		$res = safe_query($sql);
	}
//	repairall();
}
...
if ($neu) {
	...
	if ($vorlage) 	$sql = "INSERT `morp_cms_content` SET navid=$edit, vid=1, tpos=$y ,edit=1";
	else			$sql = "INSERT `morp_cms_content` SET navid=$edit, tpos=$y, tid='$tid' ,edit=1";
	$res	= safe_query($sql);

	if ($vorlage) 	{
		$sql = "INSERT `morp_cms_content_live` SET navid=$edit, vid=1, tpos=$y ,edit=1";
		safe_query($sql);
	}
}
elseif ($cid && $vorlage) {
	$sql	= "UPDATE `morp_cms_content` set vid=$vorlage ,edit=1 WHERE cid=$cid";
	$res	= safe_query($sql);
}
elseif ($copy) {
	$y 		= $pos++;
	...
	$sql	= "INSERT `morp_cms_content` SET navid=$edit, tpos=$y, theadl='$th', content='$de', tid='$tid', tlink='$tlink', tbackground='$tb', timage='$timage', theight='$theight', twidth='$twidth', tref='$tref', klasse='$klasse' , edit=1";
	$res	= safe_query($sql);
...
*/
	$sql = "UPDATE `morp_cms_content` set todelete=1 WHERE cid=$delete";
	safe_query($sql);
	// repairNav($edit);
	protokoll($uid, "content", $delete, "del");
}
elseif ($Settende) {
	...
	$sql = "UPDATE `morp_cms_content` SET tende=$set , edit=1 WHERE cid=$Settende";
	safe_query($sql);
}
elseif ($Settabstand) {
	...
	$sql = "UPDATE `morp_cms_content` SET tabstand=$set , edit=1 WHERE cid=$Settabstand";
	safe_query($sql);
}
...
elseif ($sichtbar) {
	$query  = "UPDATE `morp_cms_content` SET ton=$sichtbar , edit=1 WHERE cid=$id";
	safe_query($query);
}
...
echo setKeywords($edit, 'morp_cms_nav', 'content', 'navid');
...
'. ($warnung ? $warnung : "
	<div class=\"col-md-9 col-sm-9 col-xs-12 mb2\">
		...
		".'<a href="?neu=1&edit='.$edit.'&navid='.$edit.'" class="button"><i class="fa fa-plus"></i> Neues leeres Template erzeugen</a>
		 ...
		 <a href="#" class="button" id="showPreview"><i class="fa fa-binoculars"></i> Vorschau</a>

		 <a class="btn btn-info hideDIV"><i class="fa fa-eye"></i></a>

		 <a class="btn btn-success refreshAll" data-id="'.$edit.'"><i class="fa fa-refresh"></i></a>').'
...
while ($row = mysqli_fetch_object($res)) {
	...
	echo '
	<div class="row zeile'.($todelete ? ' col3' : '').' item'. ($on == 1 ? "" : " off") .($tende ? ' ende' : '').'" id="'.$id.'">
';
	...
	if ($vid) {
		echo '
					*** VORLAGE *** &nbsp;
					...
					'.($todelete ? '' : '<form name="form'.$vid.'" style="float:right;">
				...
				<div class="col-md-1 col-sm-1">
					'.($isLive || $todelete ? '<span ref="'.$id.'" class="button bt3 goinglive" '.($todelete ? ' data-del="'.$id.'"' : '').' id="live'.$id.'"><i class="fa fa-'.($todelete ? 'trash-o red' : 'refresh').'"></i></span>' : '<i class="fa fa-check green"></i>').'
				</div>
				<div class="col-md-1 col-sm-1">
					'.($todelete ? '' : '<a href="?del='.$id.'&edit='.$edit.'" title="Content l&ouml;schen" class="button" style="margin:0;"><i class="fa fa-trash-o"></i></a>').'
				</div>
...
';
		...
		<div class="col-md-3 col-sm-3">
					...
					'.($todelete ? '' : '
						<a href="content_edit.php?edit='.$id.'&navid='.$edit.'" title="Content editieren" class="button bt4" style="margin-left:1em;"><i class="fa fa-pencil-square-o "></i> edit</a>
...
<div class="embed-responsive embed-responsive-4by3<?php echo $_COOKIE["morp_preview"] == 1 ? '' : ' ifHide'; ?>" id="morp_preview">
	<div class="iframeBar">
		<div class="resize"><i class="fa fa-plus-square" id="morp_preview_btn"></i></div>
		<div class="closeif"><i class="fa fa-close"></i></div>
	</div>
	<iframe class="embed-responsive-item" src="../index.php?vs=1&cid=<?php echo $edit; ?>&navid=1&lan=1" allowfullscreen></iframe>
</div>
