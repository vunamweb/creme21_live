...
foreach($dir_arr as $name) {
	...
	elseif ($name_[0] == "-") {}
	elseif (preg_match("/aufzaehlung/", $name_) || preg_match("/head/", $name_) || preg_match("/box/", $name_) || preg_match("/block/", $name_) || preg_match("/zitat/", $name_) || preg_match("/^zeile/", $name_) || (preg_match("/text/", $name_) && !preg_match("/pdf/", $name_)))
