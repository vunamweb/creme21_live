...
class ModelHozmegamenuMenu extends Model {
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LINK': 
					...
					if($link) {
						...
						$this->_menuLink .= '<li><a href="' . ($link['link']) . '">' . $link['title'] . '</a></li>' . PHP_EOL;
					}
				case 'CMS': 
                    ...
                    if(isset($cms['information_id'])) {
    					...
    					$this->_menuLink .= '<li><a href="'.$cms_link.'">'.$cms['title'].'</a></li>'.PHP_EOL;
                    }
			}
	...
	public function getMenuCustomerLinkMobile($lang_id = NULL,$setting = array()) {
		...
		$item1=0;
		$lang_id = (int)$this->config->get('config_language_id');
		$this->_menuLink = '<ul id="ma-mobilemenu" class="mobilemenu originalmenu nav-collapse collapse TTT">
...
//        $html[] = '</a>';
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id,$setting) {
        ...
        //$html[] = '<div class="parentMenu">';
		if($id == 64)
		  $html[] = '<li class="nav-item dropdown dropdown_product"><a class="nav-link" data-toggle="dropdown" href="' . $link . '">';
		else
		  $html[] = '<li class="nav-item dropdown"><a class="nav-link" data-toggle="dropdown" href="' . $link . '">';
  ...
  public function drawMenuItem($children, $level = 0, $columChunk = 0, $lang_id = 1,$setting) {
        ...
        foreach ($children as $child) {
          ...
          //  if (in_array($cate_id, $currentCate)) {
            ...
            if (count($child) > 0) {
                $parameters = null;
                $link = $this->url->link('product/category', "path=".$child['parent_id']."_".$child['category_id']."");
                //$link = $this->url->link('product/category', "path=".$child['category_id']);
         
                $html.= '<a class="dropdown-item" href="' . $link . '">' . $name .'</a>';
