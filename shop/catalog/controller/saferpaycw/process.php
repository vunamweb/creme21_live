...
require_once 'SaferpayCw/Adapter/IframeAdapter.php';
...
require_once 'SaferpayCw/Adapter/WidgetAdapter.php';


class ControllerSaferpayCwProcess extends SaferpayCw_AbstractController
{
	private function redirectSetCookieIfRequired($action) {
		if($_SERVER['REQUEST_METHOD'] !== 'GET') {
			header_remove('set-cookie');
			header('Location: ' . SaferpayCw_Util::getUrl('process', $action, array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true));
			die();
		}
	}
	...
	public function notify() {
		header_remove('set-cookie');
		$dispatcher = new Customweb_Payment_Endpoint_Dispatcher(SaferpayCw_Util::getEndpointAdapter(), SaferpayCw_Util::getContainer(), array(
			0 => 'Customweb_Saferpay',
 			1 => 'Customweb_Payment_Authorization',
 		));
		$response = new Customweb_Core_Http_Response($dispatcher->invokeControllerAction(SaferpayCw_HttpRequest::getInstance(), 'process', 'index'));
		$response->send();
		die();
	}
	...
	public function payment_page_update() {
		...
		$this->response->setOutput($this->renderView(SaferpayCw_Template::resolveTemplatePath(SaferpayCw_Template::PAYMENT_FORM_TEMPLATE), $data));
	}
	...
	private function getCheckoutFormData($failedTransaction) {
		...
		if ($failedTransaction !== null && $failedTransaction instanceof SaferpayCw_Entity_Transaction) {
			$paymentMethodName = $failedTransaction->getPaymentMachineName();
		}
		else if (isset($order_info['payment_code'])) {
			$paymentMethodName = str_replace('saferpaycw_', '', $order_info['payment_code']);
		}
		else {
			die("Unable to determine the current selected payment method.");
		}

		$paymentMethod = SaferpayCw_PaymentMethod::getPaymentMethod($paymentMethodName);
	...
	public function payment() {
		...
		$data['breadcrumbs'][] = array(
			'text'      => SaferpayCw_Language::_("Payment"),
			'href'      => $this->url->link('checkout/checkout', '', 'SSL'),
			'separator' => $this->language->get('text_separator')
		);

		// Translations:
		$data['heading_title'] = SaferpayCw_Language::_("Payment");

		$failedTransaction = null;
		if (isset($_REQUEST['failed_transaction_id'])) {
			$failedTransaction = SaferpayCw_Entity_Transaction::loadById($_REQUEST['failed_transaction_id']);
		...
		$this->response->setOutput($this->renderView(SaferpayCw_Template::resolveTemplatePath('template/saferpaycw/payment_page'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
	...
	public function iframe() {
		...
		$paymentAdapter = SaferpayCw_Util::getAuthorizationAdapterFactory()->getAuthorizationAdapterByName($transaction->getTransactionObject()->getAuthorizationMethod());

		$adapter = SaferpayCw_Util::getShopAdapterByPaymentAdapter($paymentAdapter);

		if (!($adapter instanceof SaferpayCw_Adapter_IframeAdapter)) {
		...
		$adapter->prepareWithFormData(SaferpayCw_Util::getFormData(), $transaction);
	...
	public function widget() {
		...
		if (!($adapter instanceof SaferpayCw_Adapter_WidgetAdapter)) {
	...
	public function server_authorization() {
		...
		$response = new Customweb_Core_Http_Response($paymentAdapter->processAuthorization($transactionObject, SaferpayCw_Util::getFormData()));
		
		SaferpayCw_Util::getTransactionHandler()->persistTransactionObject($transactionObject);
		$response->send();
		die();
	}
	...
	public function iframe_breakout() {
		...
		$this->response->setOutput($this->renderView(SaferpayCw_Template::resolveTemplatePath('template/saferpaycw/iframe_breakout'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
			'common/content_bottom',
	...
	/**
	 * Redirect the customer to the payment page of the Saferpay.
	 */
	public function redirection() {

		$transaction = $this->getTransactionFromRequest();

		$adapter = SaferpayCw_Util::getAuthorizationAdapterFactory()->getAuthorizationAdapterByName($transaction->getTransactionObject()->getAuthorizationMethod());
		...
		$params = SaferpayCw_Util::getFormData();
		...
		if ($headerRedirection) {
			...
			$data['button_continue'] = SaferpayCw_Language::_("Continue");
			SaferpayCw_Util::getEntityManager()->persist($transaction);

			$this->template = SaferpayCw_Template::resolveTemplatePath('template/saferpaycw/redirect');
			...
			$this->response->setOutput($this->renderView(SaferpayCw_Template::resolveTemplatePath('template/saferpaycw/redirect'), $data, array(
				'common/column_left',
				'common/column_right',
				'common/content_top',
				'common/content_bottom',
	...
	public function failed() {
		...
		$url = SaferpayCw_Util::getUrl('checkout', '', array('failed_transaction_id' => $failedTransaction->getTransactionId()), true, 'checkout');
		header('Location: ' . $url);
		die();
	...
	public function success() {
		...
		while (true) {
			...
			$transaction = SaferpayCw_Entity_Transaction::loadById(intval($_REQUEST['cw_transaction_id']), false);

			$transactionObject = $transaction->getTransactionObject();

			$url = null;
			if ($transactionObject->isAuthorizationFailed()) {
				$url = SaferpayCw_Util::getUrl('process', 'failed', array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true);
			}
			else if ($transactionObject->isAuthorized()) {
				$url = SaferpayCw_Util::getUrl('success', '', array(), true, 'checkout');
			}
			...
			if (time() - $start > $maxExecutionTime) {
				$url = SaferpayCw_Util::getUrl('process', 'timeout', array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true);
				header('Location: ' . $url);
				die();
			}
			else {
	...
	public function timeout() {
		...
		$data['timeout_message'] = SaferpayCw_Language::_('It seems as your order was successful. However we do not get any feedback from the payment processor. Please contact us to find out what is going on with your order.');
		$data['order_message'] = SaferpayCw_Language::_('Please mention the following order id:');
		$data['order_id'] = $transaction->getOrderId();
		
		
		$this->response->setOutput($this->renderView(SaferpayCw_Template::resolveTemplatePath('template/saferpaycw/timeout'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
			'common/content_bottom',
	...
	private function getTransactionFromRequest() {
		...
		$transaction = SaferpayCw_Entity_Transaction::loadById(intval($_REQUEST['cw_transaction_id']));
