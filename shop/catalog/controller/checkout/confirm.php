...
class ControllerCheckoutConfirm extends Controller
{
    public function index()
    {
        ...
        if (!$redirect)
        {
            ...
            {
                $customer_info = $this->model_account_customer->getCustomer($this->customer->
                    getId());
            ...
            if (isset($this->request->cookie['tracking']))
            {
                ...
                // Affiliate
                $affiliate_info = $this->model_account_customer->getAffiliateByTracking($this->
                    request->cookie['tracking']);
                ...
                $marketing_info = $this->model_checkout_marketing->getMarketingByCode($this->
                    request->cookie['tracking']);
            ...
            foreach ($this->cart->getProducts() as $product)
            {
                ...
                $data['products'][] = array(
                    ...
                    'total' => $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'],
                        $this->config->get('config_tax')) * $product['quantity'], $this->session->data['currency']),
            ...
            if (!empty($this->session->data['vouchers']))
            {
                foreach ($this->session->data['vouchers'] as $voucher)
                {
                    $data['vouchers'][] = array('description' => $voucher['description'], 'amount' =>
                            $this->currency->format($voucher['amount'], $this->session->data['currency']));
                }
            }

            $data['totals'] = array();

            $this->document->displayOrder($order_data['totals'], $this->session->data['shipping_address']['country_id']);
            ...
            $data['payment'] = $this->load->controller('extension/payment/' . $this->
                session->data['payment_method']['code']);
        } else
        {
