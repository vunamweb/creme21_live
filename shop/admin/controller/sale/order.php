...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['pdf'] = $this->url->link('sale/order/pdf', 'user_token=' . $this->session->data['user_token'], true);
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['pdf'] = $this->url->link('sale/order/pdf', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			...
			$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
            //print_r($data['order_statuses']); die();
    ...
    public function history_new() {
	    $this->send_email_history();
        $this->history(); 
    }

	public function send_email_history() {
	   $this->load->language('sale/order');
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
                    ...
                    //echo $order_info['currency_value']; die();
                    //echo $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], '1.0000,00'); die();
                    $product_data[] = array(
						'name_1'     => $newName[0],
                        'name_2'     => $newName[1],
						'model'    => $product['model'],
				...
				//echo $this->language->get('date_format_short'); die();
                $data['orders'][] = array(
					'order_id'	       => $order_id,
                ...
                if($_GET['notify'] == '1') {
                    ...
                    $contentPdf = $this->load->view('sale/order_invoice_pdf', $data);
                    //echo$contentPdf; die();
                    //echo $contentPdf; die();
                    ...
                    $dompdf->render();
                    $pdf = $dompdf->output();
                    $file_location = "../pdf/Rechnung-".$order_info[order_id].".pdf";
                    file_put_contents($file_location,$pdf);
                    
                    $files = "../pdf/Rechnung-".$order_info[order_id].".pdf";
                    $filename = "Rechnung-".$order_info[order_id].".pdf";
                    $fp = fopen($files, "rb");
                    ...
                    $data_history['order_id'] = $order_info['order_id'];
                    $data_history['order_status'] = $order_info['order_status'];
                    $data_history['server'] = $_SERVER['SERVER_NAME'];
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
                    ...
                    $product_data[] = array(
						...
						'option'   => $option_data,
						'quantity' => $product['quantity'],
						'tax' => round($product['total'] - $product['total']/1.16, 2),
						'price'    => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
					);
				}
				...
				$this->document->displayOrderAgain($total_data, $totals);
                ...
                $dompdf->render();
                $pdf = $dompdf->output();
                $file_location = "../pdf/Rechnung-".$order_id.".pdf";
                file_put_contents($file_location,$pdf);
                
                ?>
                <script>
	               location.href='../pdf/Rechnung-<?php echo $order_id ?>.pdf';
                </script>
                <?php 
