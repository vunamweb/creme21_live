...
abstract class ControllerPaymentStripeCwAbstract extends StripeCw_AbstractController implements StripeCw_IPaymentMethodDefinition
{
	...
	public function index() {
		...
		$data['tabs'] = $this->model_stripecw_setting->renderStoreTabs($this->url->link($this->getModuleBasePath() . '_'  . strtolower($paymentMethod->getPaymentMethodName()), 'user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		$this->response->setOutput($this->renderView('stripecw/settings', $data, array(
			'common/header',
			'common/footer',
		)));
	}
	...
	public function orderAction() {
		...
		$this->response->setOutput($this->renderView('stripecw/order_form', $data));
	}
