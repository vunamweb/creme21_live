...
abstract class ControllerPaymentSaferpayCwAbstract extends SaferpayCw_AbstractController implements SaferpayCw_IPaymentMethodDefinition
{
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_saferpaycw_setting->saveSettings($paymentMethod->getSettingsApi(), $this->request->post);
		...
		$heading = SaferpayCw_Language::_("Configurations for !method (Saferpay)", array('!method' => $paymentMethod->getPaymentMethodDisplayName()));
		...
		$data['tabs'] = $this->model_saferpaycw_setting->renderStoreTabs($this->url->link($this->getModuleBasePath() . '_'  . strtolower($paymentMethod->getPaymentMethodName()), 'user_token=' . $this->session->data['user_token'], 'SSL'));
		$data['form'] = $this->model_saferpaycw_setting->render($paymentMethod->getSettingsApi());
		...
		$this->response->setOutput($this->renderView('saferpaycw/settings', $data, array(
			'common/header',
			'common/footer',
		)));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/saferpaycw')) {
	...
	public function orderAction() {
		...
		$transactions = SaferpayCw_Entity_Transaction::getTransactionsByOrderId($orderId);

		$data = array();
		$data['transactions'] = $transactions;
		$this->response->setOutput($this->renderView('saferpaycw/order_form', $data));
	}
