...
abstract class ModelSaferpayCwAbstractSetting extends Model
{
	...
	public function saveSettings(SaferpayCw_SettingApi $api, $data) {
		
		$this->api = $api;
		
		// Store default values:
		$storeIds = array(SaferpayCw_Store::DEFAULT_STORE_ID);
				...
				if ($useDefault == false) {
					...
					if ($type == 'file') {
						...
						if (isset($data['reset'][$key][$storeId]) && $data['reset'][$key][$storeId] == 'reset') {
							...
							if (!is_writable(SaferpayCw_Util::getFileUploadDir())) {
								die(Customweb_I18n_Translation::__("For uploading files the path '@path' must be writable.", array('@path' => SaferpayCw_Util::getFileUploadDir())));
							}
							move_uploaded_file($_FILES[$fileKey]['tmp_name'], SaferpayCw_Util::getFileUploadDir() . '/' . $fileName);
	...
	public function getStoresIncludingDefaultStore() {
		...
		$stores = array(array(
			'store_id' => '0',
			'name' => SaferpayCw_Language::_('Default Store'),
			'url' => $url,
			'ssl' => $ssl,
		));
	...
	protected function renderCurrencySelect(array $item, $storeId) {
		...
		$currencyActive = SaferpayCw_Util::getCurrencies();
