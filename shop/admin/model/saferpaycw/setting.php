...
class ModelSaferpayCwSetting extends ModelSaferpayCwAbstractSetting
{

	public function render(SaferpayCw_SettingApi $api) {
		$this->api = $api;
	...
	protected function renderElement($item, $controlHtml, $storeId) {
		...
		if (SaferpayCw_Store::DEFAULT_STORE_ID != $storeId) {
			$defaultValue = $this->api->getValue($item['key'], SaferpayCw_Store::DEFAULT_STORE_ID);
			if ($defaultValue == $item['value']) {
				$isDefault = true;
				$formGroupClasses .= ' saferpaycw-use-default';
			}
		}
		...
		$output .= '<label class="col-sm-2 control-label" for="' . $this->getControlId($item, $storeId) . '">
				...
				title="" data-original-title="' . SaferpayCw_Language::_($item['description']) . '">' . SaferpayCw_Language::_($item['title']) . '</span></label>';
		
		// Add default checkbox
		if (SaferpayCw_Store::DEFAULT_STORE_ID != $storeId) {
			
			$checked = '';
			$classes = 'saferpaycw-control-box';
			if ($isDefault) {
			...
			$output .= '<div class="col-sm-10 ' . $classes . '">' . $controlHtml . '</div>
				<div class="col-sm-offset-2 col-sm-10">
					<div class="checkbox">
						<label class="saferpaycw-default-box">
							<input type="checkbox" id="' . $id .'"  name="use_default[' . $item['key'] . '][' . $storeId . ']" value="true" ' . $checked . ' />
							' .  SaferpayCw_Language::_('Use default store value') . '
						</label>
					</div>
				</div>';
		}
	...
	protected function renderOrderStatusSelect(array $item, $storeId) {
		...
		$item['options'] = $item['options'] + SaferpayCw_OrderStatus::getOrderStatuses();
	...
	protected function renderFile(array $item, $storeId) {
		...
		$output .= '<label for="reset-setting-' . $item['key'] . '-' . $storeId .'">' . SaferpayCw_Language::_('Reset') . '</label></div>';
		
		$stream = $item['value'];
		if ($stream instanceof Customweb_Core_Stream_Input_File) {
			$output .= '<div class="current-file-path">' . SaferpayCw_Language::_('File Path: ') . $stream->getFilePath() . '</div>';
		}
