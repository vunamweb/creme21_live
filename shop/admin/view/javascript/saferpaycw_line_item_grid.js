(function($) {
	...
	var SaferpayCwLineItemGrid = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#saferpaycw-decimal-places").val());
			this.currencyCode = $("#saferpaycw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".saferpaycw-line-item-grid input.line-item-quantity").each(function() {
				SaferpayCwLineItemGrid.attachListener(this);
			});
			$(".saferpaycw-line-item-grid input.line-item-price-excluding").each(function() {
				SaferpayCwLineItemGrid.attachListener(this);
			});
			$(".saferpaycw-line-item-grid input.line-item-price-including").each(function() {
				SaferpayCwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				SaferpayCwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.saferpaycw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
