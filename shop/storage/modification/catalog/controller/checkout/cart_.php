        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers']))
        {
            ...
            foreach ($products as $product)
            {
                ...
                } else
                {
                    ...
                    $this->document->displayCard($product_options, $option_data);
                }
        ...
        if ($product_info)
        {
            ...
            if (!$json)
            {
                if ($this->request->post['product_mix_mask'] != '')
                    $option = $this->request->post['product_mix_mask'];
                //$option = htmlentities($option,ENT_QUOTES | ENT_IGNORE | ENT_SUBSTITUTE | ENT_DISALLOWED | ENT_HTML401 | ENT_XML1 | ENT_XHTML | ENT_HTML5, "ISO-8859-1");
                //echo $option; die();

                $this->cart->add($this->request->post['product_id'], $quantity, $option, $recurring_id);

                $json['success'] = sprintf($this->language->get('text_success'), $this->url->
                    link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'],
