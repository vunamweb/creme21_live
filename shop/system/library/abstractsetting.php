...
require_once DIR_SYSTEM . '/library/cw/PayPalPlusCw/init.php';

require_once 'Customweb/I18n/Translation.php';

require_once 'PayPalPlusCw/Language.php';
require_once 'PayPalPlusCw/Util.php';
require_once 'PayPalPlusCw/Store.php';


abstract class ModelPayPalPlusCwAbstractSetting extends Model
{
	protected $settingsDefinitions = array();
	
	/**
	 * @var PayPalPlusCw_SettingApi
	 */
	protected $api;
	
	public function saveSettings(PayPalPlusCw_SettingApi $api, $data) {
		
		$this->api = $api;
		
		// Store default values:
		$storeIds = array(PayPalPlusCw_Store::DEFAULT_STORE_ID);
				...
				if ($useDefault == false) {
					...
					if ($type == 'file') {
						...
						if (isset($data['reset'][$key][$storeId]) && $data['reset'][$key][$storeId] == 'reset') {
							...
							if (!is_writable(PayPalPlusCw_Util::getFileUploadDir())) {
								die(Customweb_I18n_Translation::__("For uploading files the path '@path' must be writable.", array('@path' => PayPalPlusCw_Util::getFileUploadDir())));
							}
							move_uploaded_file($_FILES[$fileKey]['tmp_name'], PayPalPlusCw_Util::getFileUploadDir() . '/' . $fileName);
	...
	public function render(PayPalPlusCw_SettingApi $api) {
		$this->api = $api;
		$output = '';
		foreach ($this->getStoresIncludingDefaultStore() as $store) {
	...
	public function getStoresIncludingDefaultStore() {
		...
		$stores = array(array(
			'store_id' => '0',
			'name' => PayPalPlusCw_Language::_('Default Store'),
			'url' => $url,
			'ssl' => $ssl,
		));
	...
	protected function renderCurrencySelect(array $item, $storeId) {
		...
		$currencyActive = PayPalPlusCw_Util::getCurrencies();
