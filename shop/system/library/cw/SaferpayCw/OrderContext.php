...
class SaferpayCw_OrderContext extends SaferpayCw_AbstractOrderContext
{
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod, array $order_info, array $order_totals) {
		...
		foreach ($this->productData as $key => $data) {
			$this->productData[$key]['total'] = SaferpayCw_Util::convertTo($data['quantity'] * $data['total'], $this->getCurrencyCode());
			$this->productData[$key]['total_with_tax'] = SaferpayCw_Util::convertTo($data['quantity'] * $data['total_with_tax'], $this->getCurrencyCode());
		}
		...
		// Langauge
		$this->languageCode = SaferpayCw_Language::getLanguageCodeByLanguageId($this->orderInfo['language_id']);

		// Order total
		$this->orderTotal = SaferpayCw_Util::convertTo($this->orderInfo['total'], $this->getCurrencyCode());
