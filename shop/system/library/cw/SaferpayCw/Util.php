...
require_once 'SaferpayCw/EntityManager.php';
...
require_once 'SaferpayCw/DatabaseDriver.php';
require_once 'SaferpayCw/Entity/PaymentCustomerContext.php';
require_once 'SaferpayCw/HttpRequest.php';


final class SaferpayCw_Util {
	...
	public static function getUrl($controller, $action = '', array $params = array(), $sslActive = true, $space = 'saferpaycw') {
		...
		$config = SaferpayCw_Store::getStoreConfigs();
	...
	public static function getFileUploadDir() {
		if (self::$fileUploadDir === null) {
			...
			self::$fileUploadDir = $base . 'upload/saferpaycw/';
		}
	...
	public static function getOrderTotals($registry) {
		...
		foreach ($totalData as $id => $data) {
			...
			$totalData[$id]['value'] = self::convertTo(
				$totalData[$id]['value'],
				SaferpayCw_Util::getRegistry()->get('session')->data['currency']
			);
	...
	public static function getContainer() {
		if (self::$container === null) {
			...
			$packages[] = 'SaferpayCw_LayoutRenderer';
			$packages[] = 'SaferpayCw_EndpointAdapter';
			$packages[] = 'Customweb_Payment_SettingHandler';
			$packages[] = 'Customweb_Storage_Backend_Database';
			$packages[] = 'SaferpayCw_TemplateRenderer';
			...
			$provider
				...
				->addObject(SaferpayCw_HttpRequest::getInstance())
				->addObject(self::getDriver())
				->addObject(self::getAssetResolver())
				->add('databaseTransactionClassName', 'SaferpayCw_Entity_Transaction')
				->add('storageDatabaseEntityClassName', 'SaferpayCw_Entity_Storage');
				;
	...
	public static function getEntityManager() {
		if (self::$entityManager === null) {
			...
			self::$entityManager = new SaferpayCw_EntityManager(self::getDriver(), $cache);
		}
	...
	public static function getDriver() {
		if (self::$driver === null) {
			self::$driver = new SaferpayCw_DatabaseDriver();
		}
		return self::$driver;
	}
	...
	public static function getShopAdapterByPaymentAdapter(Customweb_Payment_Authorization_IAdapter $paymentAdapter) {
		...
		$adapters = self::getContainer()->getBeansByType('SaferpayCw_Adapter_IAdapter');
		foreach ($adapters as $adapter) {
			if ($adapter instanceof SaferpayCw_Adapter_IAdapter) {
	...
	public static function getLanguages() {
		return SaferpayCw_Language::getLanguages();
	}
	...
	public static function migrate() {
		...
		$migration = new Customweb_Database_Migration_Manager(SaferpayCw_Util::getDriver(), dirname(__FILE__) . '/Migration/', DB_PREFIX.'saferpaycw_schema_version');
		$migration->migrate();
		
		$registry = self::getRegistry();
		
		$registry->get('load')->model('user/user_group');
		$registry->get('model_user_user_group')->addPermission($registry->get('user')->getId(), 'access', 'saferpaycw/transaction');
		$registry->get('model_user_user_group')->addPermission($registry->get('user')->getId(), 'modify', 'saferpaycw/transaction');
	}
	...
	public static function getPaymentCustomerContext($customerId) {
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('SaferpayCw_Entity_PaymentCustomerContext', 'loadByCustomerId', array(
			...
			if (count($entities) > 0) {
				...
				$context = new SaferpayCw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context) {
		if ($context instanceof SaferpayCw_Entity_PaymentCustomerContext) {
	...
	/**
	 * @return SaferpayCw_EndpointAdapter
	 */
	public static function getEndpointAdapter() {
		return self::getContainer()->getBean('SaferpayCw_EndpointAdapter');
	}
	...
	public static function getAssetResolver() {
		if (self::$resolver === null) {
			...
			$defaultTemplateUrl = SaferpayCw_Store::getStoreBaseUrl() . $relativeTemplatePathPart . 'default/';
			
			$configs = SaferpayCw_Store::getStoreConfigs();
			...
			$currentTemplateUrl = SaferpayCw_Store::getStoreBaseUrl() . $relativeTemplatePathPart . $currentTemplate . '/';
			
			
			self::$resolver = new Customweb_Asset_Resolver_Composite(array(
				new Customweb_Asset_Resolver_Simple(
						$currentTemplatePath . '/template/saferpaycw/snippets/',
						$currentTemplateUrl . '/template/saferpaycw/snippets/',
						array('application/x-twig')
				),
				new Customweb_Asset_Resolver_Simple(
						$currentTemplatePath . '/stylesheet/saferpaycw/',
						$currentTemplateUrl . '/stylesheet/saferpaycw/',
						array('text/css')
				),
				new Customweb_Asset_Resolver_Simple(
						$currentTemplatePath . '/javascript/saferpaycw/',
						$currentTemplateUrl . '/javascript/saferpaycw/',
						array('application/javascript')
				),
				new Customweb_Asset_Resolver_Simple(
						$currentTemplatePath . '/image/saferpaycw/',
						$currentTemplateUrl . '/image/saferpaycw/',
						array('image/png')
				),
				new Customweb_Asset_Resolver_Simple(
						$defaultTemplatePath . '/template/saferpaycw/snippets/',
						$defaultTemplateUrl . '/template/saferpaycw/snippets/',
						array('application/x-twig')
				),
				new Customweb_Asset_Resolver_Simple(
						$defaultTemplatePath . '/stylesheet/saferpaycw/',
						$defaultTemplateUrl . '/stylesheet/saferpaycw/',
						array('text/css')
				),
				new Customweb_Asset_Resolver_Simple(
						$defaultTemplatePath . '/javascript/saferpaycw/',
						$defaultTemplateUrl . '/javascript/saferpaycw/',
						array('application/javascript')
				),
				new Customweb_Asset_Resolver_Simple(
						$defaultTemplatePath . '/image/saferpaycw/',
						$defaultTemplateUrl . '/image/saferpaycw/',
						array('image/png')
				),
				new Customweb_Asset_Resolver_Simple(
						dirname(dirname($defaultTemplatePath)) . '/asset/saferpaycw/',
						dirname(dirname($defaultTemplateUrl)) . '/asset/saferpaycw/'
				),
			));
		}
