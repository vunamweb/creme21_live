 ...
 * @Bean
 */
class SaferpayCw_Adapter_WidgetAdapter extends SaferpayCw_Adapter_AbstractAdapter {
	...
	protected function preparePaymentFormPane(){
		...
		if ($this->visibleFormFields !== null && count($this->visibleFormFields) > 0) {
			$this->formActionUrl = SaferpayCw_Util::getUrl('process', 'widget', 
					array(
	...
	public function prepareWithFormData(array $formData, SaferpayCw_Entity_Transaction $transaction){
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			if (is_object($errorMessage)) {
				...
				$this->errorMessage = SaferpayCw_Language::_("Failed to initialize transaction with an unknown error");
			}
		}
	}
	...
	public function getWidget(){
		if ($this->widgetHtml !== null) {
			$id = 'saferpaycw-a' . Customweb_Util_Rand::getRandomString(50);
			return '<div class="saferpaycw-widget" id="' . $id . '"></div> <script type="text/javascript">
					var saferpaycwCallback = function() {
