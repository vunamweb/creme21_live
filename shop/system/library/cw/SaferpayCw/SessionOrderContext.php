...
require_once 'SaferpayCw/AbstractOrderContext.php';
 ...
 *
 */
class SaferpayCw_SessionOrderContext extends SaferpayCw_AbstractOrderContext
{
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod, $registry) {
		...
		if (isset($session->data['shipping_address']) && is_array($session->data['shipping_address'])) {
			$shipping = SaferpayCw_Util::mergeArray($shipping, $session->data['shipping_address']);
		}
		if (isset($session->data['shipping']['shipping_address']) && is_array($session->data['shipping']['shipping_address'])) {
			$shipping = SaferpayCw_Util::mergeArray($shipping, $session->data['shipping']['shipping_address']);

		}
		if (isset($session->data['guest']['shipping']) && is_array($session->data['guest']['shipping'])) {
			$shipping = SaferpayCw_Util::mergeArray($shipping, $session->data['guest']['shipping']);
		}
		...
		if (isset($session->data['guest']['payment']) && is_array($session->data['guest']['payment'])) {
			$payment = SaferpayCw_Util::mergeArray($payment, $session->data['guest']['payment']);

		}
		if (isset($session->data['payment_address']) && is_array($session->data['payment_address'])) {
			$payment = SaferpayCw_Util::mergeArray($payment, $session->data['payment_address']);
		}
		...
		// Order Totals
		$this->orderTotals = SaferpayCw_Util::getOrderTotals($registry);

		// Langauge
		$this->languageCode = SaferpayCw_Language::getCurrentLanguageCode();
