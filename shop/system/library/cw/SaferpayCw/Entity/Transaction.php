...
/**
 *
 * @Entity(tableName = 'saferpaycw_transactions')
 *
 */
class SaferpayCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction
{
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager) {
		if (version_compare(VERSION, '2.0.0.0') >= 0) {
			...
			if ($this->getTransactionObject()->isAuthorized()) {
				...
				$registry = SaferpayCw_Util::getRegistry();
	...
	public function onAfterLoad(Customweb_Database_Entity_IManager $entityManager) {
		...
		SaferpayCw_Store::forceStoreId($this->getStoreId());
	}
	...
	public function getStoreId() {
		if ($this->storeId === null) {
			...
			if (!empty($orderId)) {
				$rs = SaferpayCw_Util::getDriver()->query("SELECT store_id FROM `" . DB_PREFIX . 'order` WHERE order_id = >orderId')->setParameter('>orderId', $orderId);
	...
	public static function getGridQuery() {
		return 'SELECT * FROM ' . DB_PREFIX . 'saferpaycw_transactions WHERE ${WHERE} ${ORDER_BY} ${LIMIT}';
	}
	...
	public static function loadById($id, $cache = true) {
		return SaferpayCw_Util::getEntityManager()->fetch('SaferpayCw_Entity_Transaction', $id, $cache);
	}
	...
	public static function getTransactionsByOrderId($orderId, $loadFromCache = true) {
		return SaferpayCw_Util::getEntityManager()->searchByFilterName('SaferpayCw_Entity_Transaction', 'loadByOrderId', array('>orderId' => $orderId), $loadFromCache);
	}
	...
	private function updateOrderStatusFromFrontend($currentStatus) {
		$registry = SaferpayCw_Util::getRegistry();
		SaferpayCw_Util::setMockCartProducts($this->getOrderId());
		$registry->get('load')->model('checkout/order');
		if (version_compare(VERSION, '2.0.0.0') >= 0) {
