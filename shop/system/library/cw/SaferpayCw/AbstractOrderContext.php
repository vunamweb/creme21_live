...
require_once 'SaferpayCw/IncompleteDataException.php';
 ...
 *
 */
abstract class SaferpayCw_AbstractOrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	/**
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		if (!isset($_SESSION['saferpaycw_checkout_id'])) {
			$_SESSION['saferpaycw_checkout_id'] = array();
		}
		if (!isset($_SESSION['saferpaycw_checkout_id'][$paymentMethod->getPaymentMethodName()])) {
			$_SESSION['saferpaycw_checkout_id'][$paymentMethod->getPaymentMethodName()] = Customweb_Core_Util_Rand::getUuid();
		}
		$this->checkoutId = $_SESSION['saferpaycw_checkout_id'][$paymentMethod->getPaymentMethodName()];
	}
	...
	public function getShippingMethod() {
		...
		if (isset($totals['shipping']['title'])) {
			...
			return SaferpayCw_Language::_('No Shipping');
		}
	}
	...
	public function getBillingCountryIsoCode() {
		...
		if (!isset($data['iso_code_2'])) {
			throw new SaferpayCw_IncompleteDataException("No billing country code was set.");
		}
