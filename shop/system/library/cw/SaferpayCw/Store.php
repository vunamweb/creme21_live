...
final class SaferpayCw_Store {
	...
	public static function getStores() {
		if (self::$storeNames === null) {
			self::$storeNames[self::DEFAULT_STORE_ID] = SaferpayCw_Language::_("Default Store");
			$rs = SaferpayCw_Util::getDriver()->query("SELECT * FROM " . DB_PREFIX . "store ORDER BY name");
	...
	public static function getStoreConfigs($storeId = null) {
		...
		if (!isset(self::$storeConfigs[$storeId])) {
			...
			$query = SaferpayCw_Util::getDriver()->query("SELECT * FROM " . DB_PREFIX . "setting WHERE store_id = '0' OR store_id = '" . (int)$storeId . "' ORDER BY store_id ASC");
