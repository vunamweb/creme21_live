...
class ModelCatalogProduct extends Model {
	...
	public function getProductOptions_1($product_id) {
		...
		for($i = 0; $i < count($rows); $i++) {
		  ...
		  $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product_option_value vop WHERE vop.option_id = '" . (int)$rows[$i]['option_id'] . "' AND vop.product_id = '" . (int)$rows[$i]['product_id'] . "' ");
		  $rows_1 = $query->rows;
	...
	public function editProduct($product_id, $data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			...
			long_description = '" . $this->db->escape($value['long_description']) . "', product_detail = '" . $this->db->escape($value['product_detail']) . "', 
			inhaltsstoffe = '" . $this->db->escape($value['inhaltsstoffe']) . "',tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
			...
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', article_number = '" . $product_option_value['article_number'] . "' , subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					}
				} else {
	...
	public function getProductDescriptions($product_id) {
		...
		foreach ($query->rows as $result) {
			$product_description_data[$result['language_id']] = array(
				...
				'inhaltsstoffe'      => $result['inhaltsstoffe'],
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
					...
					'article_number'          => $product_option_value['article_number'],
