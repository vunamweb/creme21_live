...
class ControllerCatalogProduct extends Controller {
    ...
    public function exportExcel()
    {
        ...
        define("DB_USER", DB_USERNAME); //DB user name
        define("DB_PASS", DB_PASSWORD); //DB password
        define("DB_NAME", DB_DATABASE); //DB database name
        define("PREFIX", DB_PREFIX); //Table name prefix (if any). Example: "oc_", "opencart_", etc...
...
//Message to display in case of wrong access password
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			//get plus
			$TotalProductNotSipped = $this->model_catalog_product->getTotalProductNotSipped($result['product_id']);
			$TotalProductNotSipped = $this->showStockOption_1($TotalProductNotSipped['data'], $TotalProductNotSipped['total']);
			if($result['product_id'] == 56) {
				//print_r($this->model_catalog_product->getProductOptions_1($result['product_id']));
				//die();
			}
			  
            //print_r($TotalProductNotSipped); die();
            
            $data['products'][] = array(
				'product_id' => $result['product_id'],
				'image'      => $image,
				'name'       => $result['name'],
                'not_sipped'       => $TotalProductNotSipped,
                'model'      => $result['model'],
				'price'      => $this->currency->format($result['price'], $this->config->get('config_currency')),
				'special'    => $special,
				'quantity'   => $result['quantity'],
				'quantity_options'   => $this->showStockOption($this->model_catalog_product->getProductOptions_1($result['product_id']), $result['quantity']),
	...
	public function showStockOption($datas, $quantity) {
		$result = '';
		...
		foreach($datas as $data ) {
			if($data['value'] == 'Grösse') {
				$result .= '<p class="head">'.$data['value'].'</p>';
	...
	public function showStockOption_1($datas, $quantity) {
		$result = '';
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['type_product'])) {
			$data['type_product'] = $this->request->post['model'];
		} elseif (!empty($product_info)) {
