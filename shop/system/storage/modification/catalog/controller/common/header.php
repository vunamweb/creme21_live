...
class ControllerCommonHeader extends Controller
{
	public function index()
	{
		...
		$data['block1'] = ((int)$this->config->get('config_language_id') == 2) ? file_get_contents('../nogo/nav_de.inc') : file_get_contents('../nogo/nav_en.inc');
		$data['block1'] = str_replace(array("<?php", "?>", "nav = '", "$", "';"), '', $data['block1']);

		/* set cookie expire in 1 hour so morpheus can get basket */
		setcookie("cart", $data['cart'], time()+3600, "/"); 
		...
		if (isset($this->request->get['route'])) {
			if (isset($this->request->get['product_id'])) {
				$class = '-' . $this->request->get['product_id'] . ' not_show_top_bg';
			} elseif (isset($this->request->get['path'])) {
				$class = '-' . $this->request->get['path'] . ' not_show_top_bg';
		...
		//$data['show_top_bg'] = 'show';

		return (!(isset($_GET['cart']))) ? $this->load->view('common/header', $data) : $this->load->view('common/header_cart', $data) ;
	}
}
